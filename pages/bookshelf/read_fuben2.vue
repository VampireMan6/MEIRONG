				...
				<view class="zai-cover-v">
					...
					<view class="zai-cover-info">
						<view class="zai-cover-figure">
							<image :src="BookInfo.img" mode="widthFix" class="zai-cover-img"></image>
						</view>
		...
		methods: {
			...
			GetDetailsData(Book) {
				...
				this.BookInfo = {
					...
					sort:Book.ctitle,//类型
					intro:Book.content,
					state:Book.serialize_text,//状态
					time:newDate.toLocaleDateString(),
					news:newDate.toLocaleDateString(),//更新
					totalpage:Object.keys(Book.chapter).length
				};
				
				this.BookList = Book.chapter
				let firstchapter = Book.chapter[Object.keys(Book.chapter)[0]];
				//获取书籍内容
				this.GetReadContent(firstchapter);
			},
			GetReadContent(firstchapter) {
				// this.zaiApi({
				...
				uni.request({
					...
					success: res => {
						// console.log(JSON.stringify(res));
						this.GetReadData(res.data.content);
					},
					fail(e) {
						uni.hideLoading();
						uni.showToast({ title: '网络异常', icon: 'none' });
			...
			SetReadPage() {
				// console.log(this.BookInfo.page)
				this.pageCurrent = this.BookInfo.page;
