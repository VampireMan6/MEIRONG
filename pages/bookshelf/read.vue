<template>
	<view class="Box" style="">
		...
		<!-- 顶部结束 -->
		<!-- 小说正文开始 -->
		<view class="sview" :style="{paddingTop:'calc('+statusBarHeight+' + 40px)',color:textColor,fontSize:forUpx(size)+'px',lineHeight:forUpx(lineHeight)+'px'}">
			<rich-text :nodes="content_text"></rich-text>
		</view>
		<view class="readbottom" id="readbottom" style="display: flex;flex-direction: row;justify-content: space-around;">
			<!-- <button class="cu-btn bg-grey">上一章</button> -->
			<button class="cu-btn bg-grey nextbottom" v-if="buttonshow==true" @click="readNext">下一章</button>
		</view>
		<!-- 小说正文结束 -->
	</view>
</template>
...
export default{
	...
	onLoad(e) {
		...
		try {
			...
			this.listData = BookReadData.chapter;
			this.GetBookInfo();
		} catch (e) {
			console.log(e)
			uni.showToast({
	...
	methods:{
		...
		GetReadContent() {
			let chapterid = '';
			this.currentchapter = this.BookData.firstchapter;
			// console.log(this.BookData)
			if(!this.BookData.ischoosechatper){
				chapterid = uni.getStorageSync('chapterid'+this.BookData.id)?uni.getStorageSync('chapterid'+this.BookData.id):this.currentchapter;
			}else{
				chapterid = this.currentchapter;
				
			}
			// console.log(chapterid)
			// console.log(uni.getStorageSync('chapterid'+this.BookData.id));
			uni.showLoading({title: '加载中...'});
			
			//请求章节内容
			uni.request({
				url:ApiUrl.ApiUrl + "/api/novel/chapter",
				data:{
					id:this.BookData.id,
					key:chapterid
				},
				...
				success: res => {
					...
					this.shuming = this.BookData.title;
					this.nextchapterid = res.data.next.id;
					//记录阅读章节记录
					uni.setStorage({
						key:'chapterid'+ this.BookData.id,
						data:res.data.id
					})
		...
		readNext(){
			console.log(uni.getStorageSync('chapterid'));
			if(this.nextchapterid){
				...
				uni.showToast({title:'等待作者更新',icon:'none'});
			}
		},
		chapterlist(){
			uni.navigateTo({
				url: '/pages/profile/chapterlist'
			})
		},
		//修改正文字体大小
		changeFontSize(e){
